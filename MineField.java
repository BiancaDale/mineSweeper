...
public class MineField extends JPanel {
	...
	private final static int ROWS = 40;  // number of rows on the board
	private final static int COLS = 40;  // number of columns on the board
	...
	public void startGame(int mineCount) {
		...
		while (true) { // This loop ends when a valid board is created.
			...
			this.visit(0,0);  // Get the user started by automatically visiting the upper-left square.
	...
	private void visit( int row, int col ) {
		if (this.mined[row][col]) {  // The user has stepped on a mine and gets blown up.
			this.gameInProgress = false;
			this.userWon = false;
		}
		else { // It's OK for the user to step on this square.  Mark it as visited.
			this.mark(row,col);
			if (this.state[ROWS - 1][COLS -1] == STATE_VISITED) { // User has reached the home sqaure!
				this.gameInProgress = false;
				this.userWon = true;
			}
		}
	...
	private boolean configOK() {
		if ( this.state[ROWS-1][COLS-1] == STATE_VISITED ) {
	 ...
	 * You have to modify mark() so that in addition to marking just the one square where the user clicked, 
	 * it will also recursively mark neighboring squares as visited if it is known that they don't contain 
	 * any mines. (You only have to mark the squares by setting a value in an array; 
	 *you don't have to do any re-drawing, as that is already taken care of elsewhere.)
	 * @return 
	 */
	private void mark(int row, int col) {		
		...
		if (this.state[row][col] == STATE_VISITED) {
            return;
        }
		this.state[row][col] = STATE_VISITED;
        if (this.bombCount(row, col) > 0) {
            return;
        }
        this.mark(row - 1, col - 1);
        this.mark(row - 1, col);
        this.mark(row - 1, col + 1);
        this.mark(row, col - 1);
        this.mark(row, col + 1);
